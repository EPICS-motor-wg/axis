...
The maximum number of times the record will try again to move to the desired position. When the retry limit is reached, the record will declare the motion finished. If the desired position was not reached, the field MISS will be set to 1.
...
RMOD allows the user to select different ways of calculating the retry distance move. The default mode (Unity) moves the motor a relative distance based on the dial error (DIFF field). Badly behaved devices (piezo motors) can oscillate around their target position in this mode. Hence, two other modes were added to allow a decreasing response from the record after each retry.
...
DIFF is the difference, in engineering units, between the desired motor position, and the readback device's report of the current position.
...
RDIF is the same difference in "raw" units (normally, steps). 
...
The motor status as received from the hardware.  The MSTA bits are defined as follows:
        ...
        Unused (or slip/stall enabled)
        POSITION: closed-loop position control is enabled. (or power on)
        ...
        GAIN_SUPPORT: motor supports closed-loop position control (or supports power on).
...
The motor flags as received from the hardware.  The MFLG bits are defined as follows:
        ...
        MF_HOME_ON_LS: Homing on LS towards LS allowed (e.g. HOMF when HLS)
        MF_STOP_PROB:  Stop the axis when RA_PROBLEM is set
...
Determines NTM deadband = NTMF * (|BDST| + RDBD). NTMF must be >= 2. The NTM deadband prevents NTM logic from issuing a STOP command at the end of a DC motor move that overshoots its' target position.
...
If HLSW is not 0, the record is put into major alarm if the soft limits are violated or a limit switch is hit.

If you miss a field to put the record into minor alarm when MISS is 1, send us a pull request.
...
None of INIT, PREM or POST is currently supported.
